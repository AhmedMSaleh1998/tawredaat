...
if (!function_exists('topBrand')) {
    ...
    {
        // query 2
        $query = DB::select(DB::raw('SELECT order_items.shop_product_id, shop_products.brand_id, count(shop_products.brand_id) as brand_value_occurrence
          FROM `order_items` inner join shop_products on
         shop_products.id = order_items.shop_product_id
        ...
        $brand_name = Brand::where('id', $query[0]->brand_id)
            ->select('id')->with('translations')->first()->name;

        $percentage = ($query[0]->brand_value_occurrence /
            OrderItem::whereNotNull('shop_product_id')->count('id'))
             * 100;
...
if (!function_exists('topCategory')) {
    ...
    {
        $query = DB::select(DB::raw('SELECT order_items.shop_product_id, shop_products.category_id, count(shop_products.category_id) as category_value_occurrence
          FROM `order_items` inner join shop_products on
         shop_products.id = order_items.shop_product_id
        ...
        $category_name = Category::where('id', $query[0]->category_id)
            ->select('id')->with('translations')->first()->name;

        $percentage = ($query[0]->category_value_occurrence /
            OrderItem::whereNotNull('shop_product_id')->count('id'))
             * 100;
...
if (!function_exists('topCity')) {
    function topCity()
    {
        $query = DB::select(DB::raw('SELECT orders.user_address_id, user_addresses.city_id, count(user_addresses.city_id) as city_value_occurrence
          FROM `orders` inner join user_addresses on
         user_addresses.id = orders.user_address_id
        ...
        $city_name = City::where('id', $query[0]->city_id)
            ->select('id')->with('translations')->first()->name;

        $percentage = ($query[0]->city_value_occurrence /
            Order::count('id')) * 100;
