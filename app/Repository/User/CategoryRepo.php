...
class CategoryRepo
{
    ...
    //         $results['levelOneCategories'] = CategoryWithSubCategoriesWithoutProductsResource::collection($categories);
    ...
    public function offerBanner()
    {
        ...
        foreach($categoriesLOne as $category)
        {
            ...
            $offerProductCount = ShopProduct::where('offer' , 1)->whereIn('category_id' , $levelThreeCategories)->count();
    ...
    //     $products_ids = [];
    //     $input_specifications = request()->specifications ? request()->specifications : [];
    //     $input_brands = request()->brand ? request()->brand : [];
    //     $min_price = request()->min_price ? request()->min_price : null;
    //     $max_price = request()->max_price ? request()->max_price : null;
    //     $search = request()->search ? request()->search : null;
    //     $sort_by = request()->sort_by ? request()->sort_by : 'qty'; // Default sort by 'qty'
    //     $sort_order = request()->sort_order ? request()->sort_order : 'desc'; // Default sort order 'desc'
    //     $main_banner = CategoryBanner::where('category_id', '=', $id)->where('section', '=', '1')->first();
    
    //     $products = ShopProduct::where('category_id', $category->id)
    //         ->where('show', 1)
    //         ->select('shop_products.*')
    //         ->join('shop_product_translations', 'shop_products.id', '=', 'shop_product_translations.shop_product_id')
    //         ->when($input_specifications, function ($query) use ($input_specifications) {
    ...
    //             foreach ($specifications as $specification) {
    //                 $query->whereIn('shop_product_translations.id', ShopProductSpecification::where('specification_id', $specification)
    ...
    //     // Add brand filter to the filter array
    ...
    //     // Add price filter to the filter array
    //     array_push($filter, [
    //         'type' => __('home.price'),
    //         'key'  => 'price',
    ...
    //     // Step 2: Retrieve all specifications and their values for the entire category (before filtering)
    ...
    //     // Step 3: Apply filters to products
    //     $products = ShopProduct::where('category_id', $category->id)
    //         ->where('show', 1)
    //         ->select('shop_products.*')
    //         ->join('shop_product_translations', 'shop_products.id', '=', 'shop_product_translations.shop_product_id')
    //         ->when(!empty($input_specifications), function ($query) use ($input_specifications) {
    //                 return $query->whereIn('shop_products.id', function ($query) use ($input_specifications) {
    //                     $query->select('shop_product_id')
    //                         ->from('shop_product_specifications')
    //                         ->whereIn('specification_id', $input_specifications); // Only filter by specification IDs
    //                 });
    //             })
    //             ;
