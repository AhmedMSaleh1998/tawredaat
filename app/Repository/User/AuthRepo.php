...
class AuthRepo
{
    ...
    public function login()
    {
        ...
        try {
            ...
            //     'isVerify' => 0, 'isActive' => 1])) {
            //     return $this->result = ['validator' =>
            //         [__('auth.userNotVerfiy')], 'success' => null,
            //         'errors' => null, 'userId' => Auth::user()->id, 'status' => 105];
            // }
            // else
            if (Auth::attempt(['email' =>
                $request->input('email'),
                'password' => $request->input('password'),
                'isVerify' => 1, 'isActive' => 0])) {
                return $this->result = ['validator' =>
                    [__('auth.userNotActive')], 'success' => null,
                    'errors' => null, 'object' => null];
            } elseif (Auth::attempt(['email' =>
                $request->input('email'),
                'password' => $request->input('password'),
                'isVerify' => 1, 'isActive' => 1]) ||
                Auth::attempt(['email' =>
                    $request->input('email'),
                    'password' => $request->input('password'),
                    'isVerify' => 0, 'isActive' => 1])
            ) {
                $results['user'] = new UserResource(auth()->check()
                    ? (auth('web')->user() ?
                        auth('web')->user() : auth('api')->user()) : null);
                //create api token
                $response = General::oauthClient($request->input('email'),
                ...
                if ($response->getStatusCode() == 200) {
                    ...
                    return $this->result = ['validator' => null, 'success' => $data->access_token, 'errors' => null, 'object' => $results];
                }
                return $this->result = ['validator' => ['error in login'], 'success' => null, 'errors' => null, 'object' => null];
            } else {
                return $this->result = ['validator' => ['invalid credentials'], 'success' => null, 'errors' => null, 'object' => null];
            }
...
//                 'title' => $request->input('T') ? $request->input('T') : $request->input('title'),
//                 'company_name' => $request->input('CN') ? $request->input('CN') : $request->input('company_name'),
//                 'phone' => $request->input('phone'),
//                 'photo' => $request->input('photo') ? UploadFile::UploadSinglelFile($request->input('photo'), 'users') : null,
//                 'isVerify' => $request->input('web') ? 1 : 0,
//                 'isActive' => 1,
//             ]);
//             $results['token']  = $user->createToken('AppName')->accessToken;
// //dd($token);
//             $results['user'] = new UserResource($user);

//             // Generate a token for the newly registered user
// //            $response = General::oauthClient($user->email, $request->input('password'));
// //            if ($response->getStatusCode() == 200) {
// //                $this->saveFirebaseToken($request->all());
// //                $data = json_decode($response->getBody());
//                 DB::commit();
...
// //            dd($rfq->attachment);
// //            dd($user->name);
// //            {{sendMail($rfq->user->email,$rfq->user->name,'We Recieved your RFQ Successfully We will respond soon!',$logo,config('global.used_app_name', 'Tawredaat'),'User.mails.userRegistration');}}
...
//             // Rollback the transaction if token generation fails
//             DB::rollback();
//             return $this->result = ['validator' => ['error generating token'], 'success' => null, 'errors' => null, 'object' => null];
